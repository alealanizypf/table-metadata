document.addEventListener("DOMContentLoaded", function () {
  ...
  function generateTableMetadata(data, _columns, id) {
    ...
    data.forEach((item) => {
      ...
      if (
        item.props.hasOwnProperty("destacado") &&
        item.props["destacado"] === "Si"
      ) {
      ...
      if (item.children && item.children.length > 0) {
        ...
        item.children.forEach((child) => {
          ...
          if (child.type != "link") {
            Object.keys(child.props).forEach((keyItem) => {
              const tdText = document.createElement("td");
              tdText.colSpan = columns.length - 1;
              tdText.innerHTML = `<div><span data-colspan="6">${child.props[keyItem]}</span></div>`;
  ...
  function generateHtml() {
    ...
    containers.forEach((container) => {
      ...
      if (jsonData) {
        ...
        container.innerHTML = "<h3> No se encontró metadata</h3>";
      }
    });
  }
